...
export const PATCH = async (request: NextRequest, { params }: Params) => {
  try {
    ...
    // const { price } = json;
    const { job_id: jobId, application_id: applicantionId } = params;
    ...
    const data = z
      ...
      .safeParse({ jobId, applicantId: applicantionId });
    if (!data.success)
    ...
    const checkFreelancerApply = database
      ...
      .where("sja.id", "=", applicantionId as any);
